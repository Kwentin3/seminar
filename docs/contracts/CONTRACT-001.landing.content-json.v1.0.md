...
1. Каждый файл обязан иметь поле `schema_version` в формате SemVer.
2. `manifest.v1.json` хранит `expects` для каждого модуля.
3. Совместимость проверяется правилом:
`semver.satisfies(module.schema_version, manifest.modules[module_name].expects)`.
4. Совместимость модулей независима: несовместимость одного модуля не легализует пропуск валидации другого.
...
3. Persist выбранного значения выполняется best-effort.
...
3. После фильтра `enabled == true` квоты обязаны сохраняться:
   ...
   - `badges >= 1`
   - `cta >= 1`
4. UI не монтирует пустые контейнеры списков.
5. В v1.0 Step2 не использует `enabled` (без нового layout).
